...
DNS Go-Between is a unified DNS management service that exposes **Windows DNS, Cloudflare, AWS Route53, and Namecheap** operations through a single secure REST API and Blazor UI.

## What it does

- **Single Pane of Glass**: Manage zones and records across multiple providers via a clean Blazor web UI.
- **Multi-Provider API**: Adds and deletes supported records (`A`, `AAAA`, `CNAME`, `PTR`, `TXT`) via unified REST endpoints.
- **Granular Access Control**: Map specific Active Directory groups or users to specific DNS providers.
- **Safe Execution**: Uses local PowerShell cmdlets for Windows DNS, and official HTTP integrations for Cloudflare, Route53, and Namecheap.
- **Deep Audit Logging**: Local JSON audit trail recording all logon attempts and DNS modifications.
- **Infrastructure Friendly**: Packaged as an MSI, hosted as a Windows Service, with optional IP whitelisting.
